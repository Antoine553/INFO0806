...
predict(lin_model1, newdata = predictions, interval = "confidence")
```

```{r}
data$residuals <- residuals(lin_model1)
head(data)
```

```{r}
data$fitted <- fitted(lin_model1)
head(data)
```
...
```{r}
pred_interval <- predict(lin_model1, interval = "prediction")
...
linear_regression <- data %>% 
  ...
  annotate("text", x = 9, y = 80, 
           label = paste0("prestige = ", coef(lin_model1)["(Intercept)"] %>% round(3), 
                          " + ", coef(lin_model1)["education"] %>% round(3), " * education")) +
  theme_bw() +
  scale_x_continuous(
    breaks = seq(6, 16, 2)
  ) +
